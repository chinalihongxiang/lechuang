	...
	if(words.length == 0 || words == '请输入留言内容\n' || words == '\n'){		
		...
		$('.addNr').animate({top:'-1000px'},200,function(){
			...
			$.ajax({
				url:'/Words/send',
				type:'post',
				data:{seller_id:window.localStorage.sellerId,words: words,is_show:isShow},
...
function wordsLoad(){
	$.ajax({
		url:'/words/myWords',
		type:'post',
		data:{seller_id:window.localStorage.sellerId},
		success:function(data){
			if(data.status == 1){
...
function wordspage(n){
	$.ajax({
		url:'/words/myWords',
		type:'post',
		data:{seller_id:window.localStorage.sellerId,p:n},
		beforeSend:function(){
			$('.mask,.showL').show();
		},
		complete:function(){

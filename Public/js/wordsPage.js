...
$(document).on('click','.GoodsDetail',function(){
	...
	$(listArr).each(function(i,e){
		if(e.share_id == shareId){
			...
			var str = '<h2>'+e.title+'</h2><div class="GoodsDetailDivShowD">'+content+'</div><div class="GoodsDetailDivShowCopy" data-clipboard-target=".GoodsDetailDivShowD" shareId="'+shareId+'">一键复制</div>'+is_like+is_dislike;
		}
...
//差评
$(document).on('click','.GoodsDetailDivShowbad', function () {
	var  shareId = $(this).attr('shareId');
	dealShare(shareId,'dislike_num');
	$(this).html('已差评')
})
//留言赞
...
function shareLoad(n){
	$.ajax({
		url:'/Words/shareList',
		type:'post',
		data:{p:n,promoter_id:window.localStorage.promoterId},
		beforeSend:function(){
			$('.mask,.showL').show();
		},
		complete:function(){

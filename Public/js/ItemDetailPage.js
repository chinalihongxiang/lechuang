getpromoterId(ItemDeatailLoad);  
function ItemDeatailLoad(){
				...
				if(data.status == 1){
					...
					if(data.data.group_list.length > 0){
						$(data.data.group_list).each(function(i,e){
							if(e.group_name == '' || e.group_name == null){
								var group_name = '-  -  -  -'
							}else{
								var group_name = e.group_name.substr(0,10)
							}
							str2 +='<tr><td>'+group_name+'</td><td>'+e.time.split(' ')[0]+'<br/>'+e.time.split(' ')[1]+'</td><td>'+e.price.split('.')[0]+'元</td></tr>';
						})
					}else{
	...
	$('.searchByKey').on('click',function(){
		...
		$.ajax({
			...
			success:function(data){
				...
				$('.showL').hide();
				if(data.status == 1){
					var str = '<ul><li class="showGoodsName" data-tips="'+data.data.max_ratio.item_name+'" onclick="window.open(linkstr('+data.data.max_ratio.type+','+data.data.max_ratio.alipay_item_id+'))"><div>最高佣金比例:</div><div><div>'+data.data.max_ratio.item_name.substr(0,8)+'</div><div>'+data.data.max_ratio.ratio+'%</div></div></li><li class="showGoodsName" data-tips="'+data.data.min_ratio.item_name+'" onclick="window.open(linkstr('+data.data.min_ratio.type+','+data.data.min_ratio.alipay_item_id+'))"><div>最低佣金比例:</div><div><div>'+data.data.min_ratio.item_name.substr(0,8)+'</div><div>'+data.data.min_ratio.ratio+'%</div></div></li><li><div>平均比例:</div><div>'+data.data.avg_ratio+'%</div></li><li class="showGoodsName" data-tips="'+data.data.max_price.item_name+'" onclick="window.open(linkstr('+data.data.max_price.type+','+data.data.max_price.alipay_item_id+'))"><div>最高价格:</div><div><div>'+data.data.max_price.item_name.substr(0,8)+'</div><div>'+data.data.max_price.price+'元</div></div></li><li class="showGoodsName" data-tips="'+data.data.min_price.item_name+'" onclick="window.open(linkstr('+data.data.min_price.type+','+data.data.min_price.alipay_item_id+'))"><div>最低价格:</div><div><div>'+data.data.min_price.item_name.substr(0,8)+'</div><div>'+data.data.min_price.price+'元</div></div></li><li><div>券转化率:</div><div>'+data.data.avg_roc+'%</div></li></ul>';			
					$('.goodsInfo1').html(str);
					var str1 = '<table><tr><td>商品名称</td><td>佣金比例</td><td>价格</td><td>转化率</td><td>销量</td><td>领取量</td></tr>';
					$(data.data.item_list).each(function(i,e){
						str1 +='<tr><td><a onclick="window.open(\''+e.link+'\')">'+e.item_name.substr(0,15)+'</a></td><td>'+e.ratio+'%</td><td>'+e.price+'元</td><td>'+e.roc+'%</td><td>'+e.sale+'</td><td>'+e.take_num+'</td></tr>'
					})
					str1 +='</table>';
...
function linkstr(type,alipay_item_id){
	if(type == 1){
		var str = 'https://detail.tmall.com/item.htm?id='+alipay_item_id;
	}else if(type == 2){
		var str = 'https://item.taobao.com/item.htm?id='+alipay_item_id;
	}
	return str;
}
function saveKey(k){

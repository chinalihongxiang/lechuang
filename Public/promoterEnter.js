...
function addGroup (){
	$.ajax({
		...
		success:function(data){
			if(data.status == 1){
				var str = '<table><tr><td>排名</td><td>群名称</td><td>领券总量</td><td>总计销量</td><td>平均转化</td><td>新单个数</td><td>重复单数</td></tr>'
	...
	$.ajax({
		...
		success:function(data){
			if(data.status == 1){
				...
				$(data.data).each(function(i,e){
					str += '<tr><td>'+e.title+'</td><td><a onclick="window.open(\''+e.link+'\')" _href="'+e.link+'" class="clickIn">点击进入</a></td><td>'+e.uv+'</td></tr>'
				})
